...
const AddCream: React.FC<AddFeaturedProps> = ({
  openModal,
  onCloseModal,
  onProductAdded,
}) => {
  ...
  const handleAddProduct = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/api/products/creams", formData, {
        headers: {
      ...
      onProductAdded();
      onCloseModal();
      alert("Face cream added successfully!");
    } catch (error) {
  ...
  return (
    <Modal show={openModal} onClose={onCloseModal}>
      <Modal.Header>Add Cream</Modal.Header>
      <Modal.Body>
