...
// Add cream controller
exports.addCream = async (req, res) => {
  ...
  try {
    ...
    // Save cream data to Firebase Realtime Database
    const productRef = admin.database().ref("creams").push();
    await productRef.set(productData);

    res
      .status(201)
      .json({ message: "Cream added successfully", product: productData });
  } catch (error) {
    console.error("Error adding cream:", error);
    res.status(500).json({ error: "Failed to add cream" });
  }
};

exports.uploadMiddleware = upload;

exports.getCream = async (req, res) => {
  try {
    const productsRef = admin.database().ref("creams");
