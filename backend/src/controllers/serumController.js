...
// Add serum controller
exports.addSerum = async (req, res) => {
  ...
  try {
    ...
    // Save serum data to Firebase Realtime Database
    const productRef = admin.database().ref("serums").push();
    await productRef.set(productData);

    res
      .status(201)
      .json({ message: "Serum added successfully", product: productData });
  } catch (error) {
    console.error("Error adding serum:", error);
    res.status(500).json({ error: "Failed to add serum" });
  }
};

exports.uploadMiddleware = upload;

exports.getSerum = async (req, res) => {
  try {
    const productsRef = admin.database().ref("serums");
